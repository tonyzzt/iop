...
Process * getProcess (unsigned long inode, const char * devicename)
{
	...
	if (processMode == true) //requested PIDs by user
	{
		...
		//std::cout << "pid=" << node->pid << "hit" << endl;
		if (requestedPID == vPIDs.end()) //find
		{
			return NULL;
		}
	}
	 ...
	 * used in case the PID disappeared while iop was running
	...
	*/
	...
	processes = new ProcList (newproc, processes); //pid is saved in newproc, which is a node of global processes linklist
...
{
	unsigned long inode = conninode[connection->refpacket->gethashstring()]; //"%s:%d-%s:%d"
	if (tracemode)
		std::cout << "getProcess: " << connection->refpacket->gethashstring() <<", inode is " << inode << "\n";
	...
	if (inode == 0)
	{
		...
		if (tracemode)
			std::cout << "after refresh: " << connection->refpacket->gethashstring() <<", inode is " << inode << "\n";

		if (inode == 0) 
		{
			//std::cout << "exit since inode==0\n";
			//connection = NULL;
			// actually, all the refpacket->gethashstring must have a corresponding inode. 
			//However, it seems the inode is not show immediately in /proc/net/tcp.
			//return NULL and wait for a valid inode.
			return NULL;
		}
	...
	Process * proc;
	if (inode == 0) {
		proc = new Process (0, "", connection->refpacket->gethashstring()); //generate the pid = 0 (?) process, and name is the ip-ip.
		processes = new ProcList (proc, processes);
	} 
	else
	{
		proc = getProcess(inode, devicename);//has inode, find the corresponding process which contains the PID, pocessname
	}

	if (proc) //if proc is null, means it is filtered by PID, from user's input
		proc->connections = new ConnList (connection, proc->connections); //add ALL connections belong to the same porcess
	return proc; //proc is no much use in the caller function process_tcp
