...
void dp_parse_ip (struct dp_handle * handle, const dp_header * header, const u_char * packet)
{
	...
	if (handle->callback[dp_packet_ip] != NULL)
	{
		int done = (handle->callback[dp_packet_ip])
			(handle->userdata, header, packet); //get ip_src and ip_dst, then return false since still need tcp parser
		if (done)
			return;
	}
	switch (ip->ip_p)
...
void dp_parse_ethernet (struct dp_handle * handle, const dp_header * header, const u_char * packet)
{
	...
	if (handle->callback[dp_packet_ethernet] != NULL) //dp_packet_ethernet is not used in current project
	{
...
void dp_pcap_callback (u_char * u_handle, const struct pcap_pkthdr * header, const u_char * packet)
{
	...
	switch (handle->linktype) {
		case (DLT_EN10MB):
			dp_parse_ethernet (handle, header, packet); //get real packet in variable packet ^^, start parse
			break;
		case (DLT_PPP):
...
int dp_dispatch (struct dp_handle * handle, int count, u_char *user, int size) {
	...
	return pcap_dispatch (handle->pcap_handle, count, dp_pcap_callback, (u_char *)handle); //send the captured packet to callback function dp_pcap_callback
}
