...
void addtoconninode (char * buffer)
{
	...
	int matches = sscanf(buffer, "%*d: %64[0-9A-Fa-f]:%X %64[0-9A-Fa-f]:%X %*X %*X:%*X %*X:%*X %*X %*d %*d %ld %*512s\n",
		local_addr, &local_port, rem_addr, &rem_port, &inode); //get the inode here!!!
	...
	if (DEBUG || tracemode)
	{
		...
		std::cout << "and inode is " << inode<< std::endl;
	}

	//std::cout << "Adding to conninode\n" << std::endl;

	conninode[hashkey] = inode; //ex. "127.0.0.1:8005-0.0.0.0:0". every inode represents one connection
	...
	while (current_local_addr != NULL) {
		...
		conninode[hashkey] = inode; //42.121.126.190:8005-0.0.0.0:0
...
usleep(1000);
	...
	do
	{
		if (fgets(buffer, sizeof(buffer), procinfo))
			addtoconninode(buffer); //read per line from /proc/net/tcp
	} while (!feof(procinfo));
