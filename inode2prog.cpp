...
void setnode (unsigned long inode, prg_node * newnode) //assign pid node
{
...
void get_info_by_linkname (char * pid, char * linkname) {
	if (strncmp(linkname, "socket:[", 8) == 0) { //if matches soket then add to inodeproc[inode] mapping file
		char * ptr = linkname + 8;
		...
		free (progname);
		setnode (inode, newnode); //assign inode to the pid. Now we can use inode to get the pid from inodeproc.
	} else {
...
void get_info_for_pid(char * pid) {
	...
	while ((entry = readdir(dir))) {
		...
		get_info_by_linkname (pid, linkname); //get each line's name for "ls -al /proc/pid/fd"
		free (fromname);
	}
	closedir(dir);
	free (dirname);
...
void reread_mapping () {
	...
	while ((entry = readdir(proc))) {
		...
		get_info_for_pid(entry->d_name); //all the number represents the pid (i.e. entry->d_name). here, the pid can be network socket, but may also to a file handle...need filter
	}
	//std::cout << "End...\n";
	closedir(proc);
}
...
struct prg_node * findPID (unsigned long inode)
{
	...
	struct prg_node * node = inodeproc[inode]; //inodeproc is a inode-process mapping file
	...
	struct prg_node * retval = inodeproc[inode]; //inode 131975034, retrieve the pid and name.ex: {inode = 131975034, pid = 16569, name = "/usr/local/sbin/sshd", '\0' <repeats 491 times>}
	if (bughuntmode)
	{
		if (retval == NULL)
		{
