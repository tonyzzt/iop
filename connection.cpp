...
/*
	if (content->val->time.tv_sec == p->time.tv_sec) //merge packets node since they happens in same second
	{
...
*/
	...
	content = new PackListNode(new Packet (*p), content); //linklist, the newset to oldest next by next
}

/* sums up the total bytes used and removes 'old' packets */
u_int32_t PackList::sumanddel (timeval t , u_int32_t * packets)
{
	u_int32_t retval = 0;
	PackListNode * current = content; // the first (newest packet)
	PackListNode * previous = NULL;

	while (current != NULL) 
	{
		//std::cout << "Comparing " << current->val->time.tv_sec << " <= " << t.tv_sec - PERIOD << endl;
		if (current->val->time.tv_sec <= t.tv_sec - PERIOD) //old packet
		{
			if (current == content)
				content = NULL;
 ...
 */
void Connection::sumanddel (timeval t, u_int32_t * recv, u_int32_t * sent, u_int32_t * packet_recv, u_int32_t * packet_sent)
{
    (*sent)=(*recv)=(*packet_sent)=(*packet_recv)=0;

    *sent = sent_packets->sumanddel(t, packet_sent);
    *recv = recv_packets->sumanddel(t, packet_recv);
}
